...
function writeLineWithIconsReplacedWithSpaces(context, line, x, y, scale, family, boldSize, forceRTL) {
  ...
  // --- THIS IS THE RTL FIX ---
  // We reverse the array of words if it's RTL
  // This makes the loop draw them in the correct visual order (right-to-left)
  if (forceRTL) {
    words.reverse();
  }
  // --- END OF FIX ---
    ...
    while (word) {
      ...
      if (match) {
        ...
        if (
          ...
          // This logic is what applies the strong bold
